...
strcmp:
    ...
    mov r4, #0 /* result will be here so we set to 0 */
...
strcmp_loop:
    ...
    beq equal
    cmp r2, r3 
    movgt r4, #1 /* if first string is bigger then return 1 */
    cmp r2, r3
    blt minus_one /* calculate minus 1 */
    b exit

minus_one:
    mvn r4, #1 /* calculate 1s complement */
    add r4, r4, #1 /* add one for two complemet */
    b exit

equal:
    cmp r2, #0 /* if they are equal and one of the two is 0, then both are 0 */
    bne strcmp_loop
